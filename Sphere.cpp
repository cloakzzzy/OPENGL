...
void Engine::Entity::Sphere::GenerateModel(int acc) {
	...
	for (int i = 0; i < layers; i++) {
		...
		for (int j = 0; j < acc; j++) {
			...
			SphereVertices.push_back(pa[0]); SphereVertices.push_back(pa[1]); SphereVertices.push_back(pa[2]);
		}
	}

	SphereVertices.push_back(cx);
	SphereVertices.push_back(cy - r);
	SphereVertices.push_back(cz);

	
	int size = SphereVertices.size() / 3;
