...
void main()
{
    ...
    vec3 p = pos;
    //pitch
    pos.x = (cos(rot.y * RAD) * p.x) - (sin(rot.y * RAD) * p.y);
    pos.y = (sin(rot.y * RAD) * p.x) + (cos(rot.y * RAD) * p.y);
    vec3 p1 = pos;
    //yaw
    pos.x = (cos(rot.z * RAD) * p1.x) - (sin(rot.z * RAD) * p1.z);
    pos.z = (sin(rot.z * RAD) * p1.x) + (cos(rot.z * RAD) * p1.z);
